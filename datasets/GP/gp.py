...
class GPSampler(object):
    ...
    def __init__(self, parameters: Parameters):
        ...
        self.fmax = output.loc.detach()[idx_max]
        idx_min = output.loc.detach().argmin().item() #This is the index of the min y
        self.fmin = output.loc.detach()[idx_min]
        self.min_loc = self.input[idx_min]
    ...
    def get_y(self, X: np.ndarray) -> None:
        return self.likelihood(self.model(torch.tensor(X).double())).mean.detach().unsqueeze(-1)
